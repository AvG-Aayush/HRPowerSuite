...
export class GPSService {
  ...
  private static timeoutMs = 20000; // 20 seconds
  private static maxAgeMs = 60000; // 1 minute
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            ...
            if (gpsError.canRetry && attempts < this.maxAttempts) {
              console.warn(`GPS attempt ${attempts} failed, retrying in ${attempts * 2} seconds...`);
              setTimeout(attemptLocation, attempts * 2000); // Progressive delay: 2s, 4s, 6s
            } else {
              reject(gpsError);
            }
          },
